...
<!-- This sentence is unclear to me. Do you mean - "We wish to know the effectiveness of the medication in the target population given that it depends on the two identified effect modifiers: 1) stress level; and 2) whether or not the patient is taking medication for something other than high blood pressure." Alternatively, "We wish to know if the effectiveness of the medication depends on two potential effect modifiers: 1) stress level; and 2) whether or not the patient is taking medication for something other than high blood pressure." -->
...
-   The estimated subgroup treatment effect within each (marginal) level of each effect modifier and its estimated standard error <!-- This reads as if we are accomodating categorical and/or ordinal effect modifiers. I don't believe we are. Suggest revising to "within both levels" to make it clear that the effective models are dichotomous -->
...
<!-- Right now, this paragraph is a bit unclear to me, but I'm not sure if it is because I can't run the study example -->
...
On the other hand, the target data may be individual patient-level data or aggregate data. For the former, effect modifiers should still be dichotomized in the target data. For the latter, provide the proportion of 1s for each dichotomized effect modifier, along with the sample size. It is important that effect modifiers are dichotomized the same way in both the study and the target data. If aggregate-level target data is provided, you may also choose a correlation structure of the effect modifiers in the form of a matrix; if desired, provide a correlation (not variance) matrix of the dichotomized effect modifiers. In this example, the target data is formatted as follows.

<!-- Just want to confirm that we do not require the correlation matrix for any interpolated g-computation analyses? -->
