...
<!-- Quang -- do we still call g-computation weighting-based? What do you mean by mergeable here? I think we should highlight that this can be used when IPD is mergeable or non mergeable. -->


## Brief introduction to g-computation

In transportability and generalizability analysis, g-computation proceeds largely the same as in causal inference. 
<!-- I don't think "in causal inference" is the right term -- this is still causal inference just using g computation instead of IOPW.  -->
...
<!-- This sentence is unclear to me. Do you mean - "We wish to know the effectiveness of the medication in the target population given that it depends on the two identified effect modifiers: 1) stress level; and 2) whether or not the patient is taking medication for something other than high blood pressure." Altenratively, "We wish to know if the effectiveness of the medication depends on two potential effect modifiers: 1) stress level; and 2) whether or not the patient is taking medication for something other than high blood pressure." -->
...
To show the results of the analysis, use `summary` like you would for `lm` when fitting a linear model. Using `summary` will print out the names of the treatment and response variables, and summaries of the fitted outcome model and the final marginal structural model (MSM). Note that scientific conclusions should only be drawn from the MSM.
